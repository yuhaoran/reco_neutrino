...
#endif
#else
    if (rank == 0) write(*,*) "Calling btran_r2c" 
        call btran_c2r(slab, slab, "tff")
    if (rank == 0) write(*,*) "Done btran_r2c" 
#endif
...
!-----------------------------------------------------------------------!
  ...
  subroutine deltafield
    ...
    if (rank == 0) write(*,*) "Inside deltafield"
    ...
    if (rank == 0) write(*,*) "Calling powerspectrum" 

    call powerspectrum(pkm)

    !! Calculate matter delta field statistics

    if (rank == 0) write(*,*) "Calling di_fftw" 

    call di_fftw(-1)

    if (rank == 0) write(*,*) "Done calling di_fftw" 
...
!! for more than 1+1e6 processes this will have to be increased
    ...
    if (rank == 0) write(*,*) "Entering potentialfield"
    ...
    if (rank == 0) write(*,*) "Calling di_fftw(-1)" 

       !! Inverse FFT potential kernel
       call di_fftw(-1) 

    if (rank == 0) write(*,*) "Done calling di_fftw(-1)"
...
!       endif
    ...
    if (rank == 0) write(*,*) "Calling di_fftw(-1) #2"

    !! Inverse FFT potential field
    call di_fftw(-1)

    if (rank == 0) write(*,*) "Done calling di_fftw(-1) #2"
...
!! for more than 1+1e6 processes this will have to be increased
    ...
    if (rank == 0) write(*,*) "About to dm_zip"
...
subroutine zip_checkpoint
    ...
    write(10) np_local,scalefactor,0.0,-3./sqrt(scalefactor),0,1000.0,1000.0,1000.0,2,2,2,1.,v_r2i,0.,0.,0.
    write(11) np_local,scalefactor,0.0,-3./sqrt(scalefactor),0,1000.0,1000.0,1000.0,2,2,2,1.,v_r2i,0.,0.,0.
    do k = 1, nm_node_dim
        do j = 1, nm_node_dim
