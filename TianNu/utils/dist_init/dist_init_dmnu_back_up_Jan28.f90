...
!  equivalence (phi,slab_work,recv_cube,send_buf)
  equivalence (slab_work,phi,recv_cube,send_buf)
  equivalence (slab,cube,xp_buf,hoc)
  equivalence (slab_cache, ll, recv_buf)
  equivalence (recv_buf, ll, slab_cache)
...
!  common / equiv3  / recv_buf
  common / zvar / rhoc_i4, pos_i1, vel_i2
#endif
  common / xvar / xvp
  
!  if (rank == 0) print*,'<<Speed Up>>' 
  call mpi_initialize
...
subroutine pack_pencils
	...
	breakup = 2 !! Testing to see if reducing message size fixes problem
    num_elements = num_elements_i8 / breakup

	write(*,*) "num_elements = ", num_elements, num_elements_i8, breakup, rank 
    ...
    enddo
        ...
        write(*,*) "done isend/irecv in pack_pencils", rank
...
! initialize plan variables for fftw
    ...
    if (firstfftw) then
      ...
      print*,'1st fftw, mking plan done<'
#else
      print*,'#X fftw, mking plan>'
      ...
      print*, '#X fftw, mkding plan<'
#endif
#else
        pen_dims = (/dim_y,dim_z/)
        if (rank == 0)  print*, '[P3DFFT]setup>'
        call p3dfft_setup(pen_dims, nc, nc, nc, .true.)
        if (rank == 0) print*, '[P3DFFT]get_dims>'
        call p3dfft_get_dims(istart, iend, isize, 1, mypadd)
        if (rank == 0) print*, '[P3DFFT]get_dim2>'
...
#else
      if (rank == 0) print*, '[P3DFFT]pack_pencils>'
...
#endif
#else
        print*, '[P3DFFT]ftran_r2c>', rank
        call ftran_r2c(slab, slab, "fft")
	print*, '[P3DFFT]DONE ftran_r2c>',rank
#endif
    else
#ifdef SLAB
#ifdef SFFTW3
...
#endif
#else
        if (rank == i) print*, '[P3DFFT]clean>'
        call p3dfft_clean
        if (rank == i) print*, '[P3DFFT]cleaned<'
#endif
...
!!------------------------------------------------------------------!!
  ...
  subroutine transferfnc
    ...
    do i=0,nodes-1
      ...
      if (rank == i) print*, '[MPI_barrier:>](tf)'
      call mpi_barrier(mpi_comm_world,ierr)
      if (rank == i) print*, '[MPI_barrier:<](tf)'
    enddo 
#endif 
